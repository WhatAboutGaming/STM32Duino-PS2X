...
#define PS2X_DEBUG // Debug is actually needed, otherwise, the STM32 will poll the controller too fast, and the controller will just not understand what's going on, so debug and pointless prints are needed to slow down the processor
#define PS2X_COM_DEBUG // Debug is actually needed, otherwise, the STM32 will poll the controller too fast, and the controller will just not understand what's going on, so debug and pointless prints are needed to slow down the processor
